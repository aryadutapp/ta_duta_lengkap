...
class Dobot:
    ...
    def move_to(self, x, y, z, r, mode,wait=False):
        move_mode = PTPMode.JUMP_XYZ

        # Check mode value and assign move_mode accordingly
        if mode == 0:
            move_mode = PTPMode.JUMP_XYZ
        elif mode == 1:
            move_mode = PTPMode.MOVJ_XYZ
        elif mode == 2:
            move_mode = PTPMode.MOVL_XYZ
        elif mode == 3:
            move_mode = PTPMode.JUMP_ANGLE
        elif mode == 4:
            move_mode = PTPMode.MOVJ_ANGLE
        elif mode == 5:
            move_mode = PTPMode.MOVL_ANGLE
        elif mode == 6:
            move_mode = PTPMode.MOVJ_INC
        elif mode == 7:
            move_mode = PTPMode.MOVL_INC
        elif mode == 8:
            move_mode = PTPMode.MOVJ_XYZ_INC
        elif mode == 9:
            move_mode = PTPMode.JUMP_MOVL_XYZ
        else:
            move_mode = PTPMode.JUMP_XYZ
        self._set_ptp_cmd(x, y, z, r, mode=move_mode, wait=wait)
